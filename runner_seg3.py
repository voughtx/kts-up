#!/usr/bin/env python3
"""runner_seg3.py — PROPER segment test: master -> variant (GCM decrypt) -> media playlist -> real segment -> direct vs relay."""
...
for u in urls:
    ...
    if st3 == 200 and b"#EXTM3U" in b3[:200]:
        master = b3; print("master:", len(master), "STREAM-INF:", b"STREAM-INF" in master, flush=True); break

# variant decrypt (GCM) — highest resolution
variant = ""
for ln in master.decode("utf-8", "replace").splitlines():
    ...
    if ln2.startswith("enc2:"):
        ...
        if d.startswith("http"):
            variant = d
            break
print("variant decrypt:", "OK " + variant[:60] if variant else "FAIL", flush=True)
if not variant:
    sys.exit(1)

# media playlist via relay
st4, b4 = relay(variant)
print("media playlist:", st4, len(b4), flush=True)
if st4 != 200:
    sys.exit(1)
...
for ln in media.splitlines():
    ...
    if ln2.startswith("enc2:"):
        ...
        segs.append(ln2)
print("REAL segments:", len(segs), flush=True)
if not segs:
    print("media head:", media[:300], flush=True)
    sys.exit(1)

# test first 3 real segments: direct first, relay second
for i, su in enumerate(segs[:3]):
    t0 = time.time()
    st5, b5 = direct_bin(su)
    dt = time.time() - t0
    print(f"seg{i} DIRECT: {st5} len={len(b5)} mpegts={b5[:2]==b'\\x47'} {dt:.1f}s", flush=True)
    if st5 != 200 or b5[:2] != b"\x47":
        t0 = time.time()
        st6, b6 = relay(su)
        dt2 = time.time() - t0
        print(f"seg{i} RELAY : {st6} len={len(b6)} mpegts={b6[:2]==b'\\x47'} {dt2:.1f}s", flush=True)
