...
def _tg_base():
    ...
    for b in cands:
        try:
            gm=_req.get(f"{b}/{K1}/getMe",timeout=20)
            gj=gm.json()
            if gj.get("ok"):
...
def main():
    ...
    if not K3:
        ...
        _y.exit(1)
    try:
        gm=_req.get(f"{_TBASE}/{K1}/getMe",timeout=30)
        gj=gm.json()
    ...
    if perm:
        try:
            _req.post(f"{_TBASE}/{K1}/editMessageCaption",data={"chat_id":K2,"message_id":mid,"caption":cap+f"\n\U0001F4BE Permanent: {perm}","parse_mode":"HTML"},timeout=60)
        except Exception:
            pass
