#!/usr/bin/env python3
# Internal task automation utility.
# For internal pipeline use only. No public documentation.
...
# ---- encoded constants ----
_A = _d("aHR0cHM6Ly9hcGkua2FydG9vbnMubWUvYXBp")              # api base
_K = _d("YmNhOWUwZGYxYTVhYmIzMjkwNmNhM2Y2M2FjMDRjZWY=")      # cbc key
_G = _d("cG1TMENBTUcxUnVxNDlXYk15aEUzZmgxc091TFlFTDlydEZhellZbGpWSTJqNEJQU29nNzNoVzdBN3hNaGNlSEQwaXdyUHJWVkRYTHZ4eVdy")  # gcm secret
_TB = _d("aHR0cHM6Ly9rYXRmaWxlLW9ubGluZS12aWRlby1kb3dubG9hZGVyLmNvbQ==")  # kat base
_TT = _d("YjRjM2Q5ZTI3ZGVmNTg0ZjM4ODE0MmU2ZjM1MDRjNDRjM2QyZDEzMzhlY2MyNDYx")  # kat token
_TG = _d("aHR0cHM6Ly9hcGkudGVsZWdyYW0ub3JnL2JvdA==")       # tg api
_REF = _d("aHR0cHM6Ly9rYXJ0b29ucy5tZS8=")
_WEB = _d("aHR0cHM6Ly9rYXJ0b29ucy5tZS9wbGF5ZXI/")
_UA = ("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 "
       "(KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36")

# ---- env (generic names) ----
K1 = _o.environ.get("KEY_1", "")   # bot token
K2 = _o.environ.get("KEY_2", "")   # chat id
K3 = _o.environ.get("KEY_3", "").strip()  # auth token
K4 = _o.environ.get("KEY_4", "").rstrip("/")  # url server
K5 = _o.environ.get("KEY_5", "")   # label
K6 = _o.environ.get("KEY_6", "s1")  # target list (set via secret)
K7 = _o.environ.get("KEY_7", "")   # db uri (optional)
...
def _mk_link(url, title, q_label, filename=None):
    ...
    body = _j.dumps({
        "pageUrl": url, "url": url, "type": "hls",
        "referer": _REF, "origin": _REF.rstrip("/"),
        "cookie": "", "userAgent": _UA, "filename": fname,
    }).encode()
    _p(f"\n[*] converting ({fname}.mp4)...")
    r = _q.Request(_TB + "/api/convert", data=body, method="POST",
                   headers={"User-Agent": _UA, "X-API-Token": _TT,
    ...
    for i in range(120):
        _t.sleep(5)
        try:
            r = _q.Request(_TB + f"/api/jobs/{job}",
                           headers={"User-Agent": _UA, "X-API-Token": _TT})
...
def _del_job(job_id):
    try:
        r = _q.Request(_TB + f"/api/jobs/{job_id}", method="DELETE",
                       headers={"User-Agent": _UA, "X-API-Token": _TT})
        with _q.urlopen(r, timeout=20) as resp:
    ...
    def __init__(self):
        ...
        if K7 and _HM:
            try:
                ...
                _p(f"[!] db fail ({str(ex)[:50]}) — local file")
        self._load()
        _p("[ok] local file storage")
...
def _meta(eid):
    ...
    return {"title": d.get("title") or "", "image": d.get("image") or "",
            "season": snum, "episode": d.get("episodeNumber") or d.get("episode_number"),
            "show_title": stitle}
    ...
    for off in range(len(shows)):
        ...
        for so in range(len(seasons)):
            ...
            for eo in range(len(eps)):
                ...
                return {"id": ep["_id"], "show": show, "season": season,
                        "ep": ep, "meta": m, "ovr": False,
                        "seasons": seasons, "si": ss, "ei": (e_idx + eo) % len(eps),
                        "eps": eps}
        _store.mark_done(si)
        _store.state["i1"] = 0
        _store.state["i2"] = 0
...
def _push(url, caption, thumb=None):
    ...
    payload = {"chat_id": K2, "video": url, "caption": caption,
...
def _split_send(link, base, cap, thumb):
    ...
    _s.run(["gh", "release", "create", tag, "--repo", _o.environ.get("GITHUB_REPOSITORY", ""),
            "--title", tag, "--notes", "temp"], check=False, capture_output=True)
    outd = "/tmp/parts"
    ...
    for i, p in enumerate(parts, 1):
        asset = f"{base}.{i:03d}"
        _s.run(["gh", "release", "upload", tag, f"{outd}/{p}",
                "--repo", _o.environ.get("GITHUB_REPOSITORY", ""),
                "--clobber", f"--name={asset}"], check=False, capture_output=True)
    ...
    _s.run(["gh", "release", "delete", tag, "--yes",
            "--repo", _o.environ.get("GITHUB_REPOSITORY", "")], check=False, capture_output=True)
    _o.remove(tmp)
    return results
...
def main():
    ...
    if size and size > _SPLIT:
        ...
        if not results:
            ...
            _del_job(job)
            _y.exit(1)
        _store.save_item({"id": eid, "show": meta.get("show_title", ""),
                          "season": meta.get("season"), "episode": meta.get("episode"),
                          "title": meta.get("title", ""), "quality": q,
                          "parts": results, "web": web,
    ...
    if perm:
        try:
            _req.post(f"{_TG}/{K1}/editMessageCaption",
                      data={"chat_id": K2, "message_id": mid,
                            "caption": cap + f"\n\U0001F4BE Permanent: {perm}",
