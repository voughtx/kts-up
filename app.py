...
def _push_telethon(path,caption,thumb=None,name="video.mp4"):
    ...
    try:
        client=TelegramClient(StringSession(_KSESS),_KID,_KHASH)
        _p("[*] telethon: connecting user account...")
        client.start()
        _p("[*] telethon: uploading (2GB limit)...")
        ent=client.get_entity(int(K2))
        msg=client.send_file(ent,path,caption=caption,parse_mode="html",thumb=thumb_path or None)
        fid=""
