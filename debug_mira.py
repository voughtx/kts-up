#!/usr/bin/env python3
"""debug_mira.py — Miraculous S5E16 exact _local_convert reproduction.
Runner pe KEY_10 hai → enc2 decrypt → segments download (relay) → ffmpeg remux.
Diagnostic only."""
...
EID = "6849900e6ed2282cba655f35"  # Miraculous S5E16
...
import glob
files = sorted(glob.glob(f"{seg_dir}/*"))[:4]
for f in files:
    print("   ", f, os.path.getsize(f), "magic:", open(f, "rb").read(3), flush=True)

out = "/tmp/kts_conv.mp4"
if os.path.exists(out): os.remove(out)
cmd = (f"ffmpeg -y -hide_banner -loglevel error -allowed_extensions ALL "
...
print(rr.stderr[-2500:], flush=True)
print("out exists:", os.path.exists(out), flush=True)
if os.path.exists(out):
    print("out size:", os.path.getsize(out), flush=True)
# ffprobe segments ki streams
for f in files[:2]:
    pr = subprocess.run(f"ffprobe -v error -show_entries stream=codec_type,codec_name -of csv {f}", shell=True, capture_output=True, text=True, timeout=30)
    print(f"  probe {f}: {pr.stdout.strip()}", flush=True)
