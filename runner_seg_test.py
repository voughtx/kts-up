#!/usr/bin/env python3
"""runner_seg_test.py — runner se segment CDN direct download test (bina relay)."""
...
for u in urls:
    ...
    if st3 == 200 and b"#EXTM3U" in b3[:200]:
        master = b3; print("master via relay:", len(master), flush=True); break
    print("master relay fail:", st3, flush=True)

# segments: direct FIRST
seg_urls = []
for ln in master.decode("utf-8", "replace").splitlines():
    ln2 = ln.strip()
    if ln2.startswith("enc2:"):
        d = dec_gcm(ln2)
        if d.startswith("http"): seg_urls.append(d)
    elif ln2.startswith("http") and not ln2.startswith("#"):
        seg_urls.append(ln2)
print("segments:", len(seg_urls), flush=True)

ok = 0
for i, su in enumerate(seg_urls[:5]):
    st4, b4 = direct_bin(su)
    ts = b4[:2] == b"\x47"
    print(f"  seg{i} DIRECT: {st4} len={len(b4)} mpegts={ts}", flush=True)
    if st4 == 200 and ts: ok += 1

print(f"DIRECT segments OK: {ok}/5", flush=True)
