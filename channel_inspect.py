#!/usr/bin/env python3
# channel_inspect.py — read channel messages 1662-1695 and print content
import os, sys, json, asyncio, urllib.request as q

def log(*a): print("[insp]", *a, flush=True)
...
async def main():
    ...
    try:
        ...
        ids = list(range(1662, 1696))
        msgs = await cli.get_messages(ch, ids=ids)
        found = {}
        for m in msgs:
            if m is not None:
                found[m.id] = m
        log(f"found {len(found)}/{len(ids)} messages")
        for mid in sorted(found.keys()):
            ...
            try:
                ...
                elif m.text and not (m.media): media = "TEXT"
            except Exception:
                pass
            cap = ""
            try:
